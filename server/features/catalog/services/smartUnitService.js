...
export const createSmartUnit = async (
  ...
  try {
    if (stock > 0) {
      await createSmartUnitInstances(
        smartUnit,
        stock
      );
    }
  } catch (error) {
    await SmartUnit.findByIdAndDelete(
      smartUnit._id
    );
...
export const getSmartUnits =
  async () => {
    ...
    const instanceCounts =
      await SmartUnitInstance.aggregate([
        {
          $match: {
            smartUnit: {
              $in: smartUnitIds,
...
export const updateSmartUnit =
  async (
    ...
    const currentInstanceCount =
      await SmartUnitInstance.countDocuments(
        {
          smartUnit: id,
        }
      );
    ...
    const smartUnit =
      await SmartUnit.findByIdAndUpdate(
        id,
        updateData,
        {
          new: true,
...
export const deleteSmartUnit =
  async (id) => {
    ...
    return SmartUnit.findByIdAndDelete(
      id
    );
  };
